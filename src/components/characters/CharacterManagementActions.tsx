...
export default function CharacterManagementActions({ character, onUpdated, onDeleted }: { character: Character; onUpdated: (character: Character) => void; onDeleted: () => void }) {
  ...
  const open = (nextMode: Exclude<Mode, null>) => { setValues(valuesFrom(character)); setLinks(character.linkItems ?? []); setSessionKeys(character.sessionKeys); setStatus(''); setMode(nextMode); };
  const save = async () => {
